...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([api.getLabs(), api.getTopics()])
      .then(([labData, topicData]) => {
        setLabs(labData);
        setTopics(topicData);
      })
      .catch(console.error)
    ...
    <div className="fade-in">
      ...
      <section style={{
        ...
        {[
          ...
          { icon: Layers, label: 'Topics', value: topics.length, color: 'var(--color-info)' },
          { icon: Star, label: 'Integration', value: integrationLabs.length + (filter === 'all' ? 0 : 0), color: 'var(--color-warning)' },
          { icon: Zap, label: 'CLI Steps', value: labs.reduce((s, l) => s + (l.step_count || 0), 0), color: 'var(--color-success)' },
        ].map((stat, i) => (
      ...
      <div className="lab-grid">
        {regularLabs.map((lab, idx) => (
          <Link key={lab.slug} to={`/lab/${lab.slug}`} style={{ textDecoration: 'none' }}>
            <div className={`card fade-in stagger-${(idx % 6) + 1}`} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
